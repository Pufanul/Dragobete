const correctAnswers = ["Bogat", "Termos", "Daria", "Luna", "Beton", "Fericit", "Peste", "Caca"];
const questions = [
  "Cum este un om cu mulți bani sau care le are pe Bianca și Adela?",
  "Recipient în care se păstrează ceaiul cald la drumeții",
  "Cum o mai cheamă pe Adela?",
  "Cine latră seară de seară când o bagi pe Adela la somn?",
  "Ce rezultă din amestecul apă + nisip + ciment + pietriș?",
  "Sinonim pentru bucuros",
  "Ce este somonul?",
  "Ce face Adela o dată la 3 zile?"
];
    ...
    if (allCorrect) {
      ...
      // Apply transition for paragraph tag removal
        ...
        setTimeout(() => {
            ...
            const shiftingDistances = [93, 31, 93, 124, 93, 62, 62]; // Shifting distances for each row

            // Apply transition for shifting rows after a delay
            setTimeout(() => {
                for (let i = 1; i < allRows.length; i++) {
                    allRows[i].style.marginLeft = `${shiftingDistances[i - 1]}px`; // Set margin-left for shifted rows
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    const inputsToColor = [
                        allRows[0].querySelectorAll('.answer')[4], // 4th input of 1st row
                        allRows[1].querySelectorAll('.answer')[1], // 3rd input of 2nd row
                        allRows[2].querySelectorAll('.answer')[3], // 1st input of 3rd row
                        allRows[3].querySelectorAll('.answer')[1], // 3rd input of 4th row
                        allRows[4].querySelectorAll('.answer')[0],  // 2nd input of 5th row
                        allRows[5].querySelectorAll('.answer')[1],  // 2nd input of 5th row
                        allRows[6].querySelectorAll('.answer')[2], // 2nd input of 5th row
                        allRows[7].querySelectorAll('.answer')[2]  // 2nd input of 5th row
                    ];
